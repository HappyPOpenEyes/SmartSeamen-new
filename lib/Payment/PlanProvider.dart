  ...
  Future<bool> callGetPlanListapi(header) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < getPaymentResponse.data.plans.length; i++) {
        planAmount.add(getPaymentResponse.data.plans[i].planAmount.toString());
        ...
        planJobViews.add(getPaymentResponse.data.plans[i].jobsViews.toString());
        planStripePrice.add(getPaymentResponse.data.plans[i].stripePrice ?? '');
